...
output_cols = [
    ...
    "url",
    "source",
    "publish_time"  # 时间列固定放在最后
]
df_output = df[output_cols].copy()

# 输出前校验字段，确认存在
print("\n输出前字段列表：", df_output.columns.tolist())

# ==============================
# 13. 最终校验
# ==============================
print("\n==============================")
jp_residual = df_output[df_output["content"].str.contains(r"[\u3040-\u30ff\u30a0-\u30ff]", regex=True)]
print("校验剩余日文条数：", len(jp_residual))
print("清洗完成总数据量：", len(df_output))
print("\n前3行预览（含publish_time）：")
...
print(f"\n 处理完成！输出文件已包含publish_time列，路径：{OUTPUT_PATH}")