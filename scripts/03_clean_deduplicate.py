...
from utils.cleaners import clean_all
...
# 5. 执行综合清洗（含非中文过滤）
# ==========================================
log_info("[PROCESS] 执行综合清洗（含非中文过滤）...")
before_clean = len(df)
...
# 统计被过滤掉的数量（cleaned_body为空）
empty_after = df['cleaned_body'].isna() | (df['cleaned_body'] == '')
filtered_count = empty_after.sum()
stats['过滤非中文'] = filtered_count
log_info(f"[OK] 非中文内容过滤: {filtered_count} 条")
...
# ==========================================
# 11. 低频词过滤（已跳过，避免 sklearn 导入卡死）
# ==========================================
print("[INFO] 低频词过滤已跳过（测试数据量小）")
...
log_info(f"  过滤非中文: {stats.get('过滤非中文', 0)} 条")
