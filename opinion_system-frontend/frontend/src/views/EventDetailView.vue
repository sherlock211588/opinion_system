...
async function loadReport() {
  ...
  error.value = ''
  try {
    const [nextReport, relatedNews] = await Promise.all([
      fetchEventDetailWithFallback(eventId),
    ...
    console.error('加载事件报告失败', e)
    error.value = '事件报告加载失败，请稍后重试'
  } finally {
    loading.value = false
  }
}
...
watch(
  report,
  (r) => {
    if (!r?.event?.title) return
    addRecentView({
      id: r.id,
      type: 'report',
      title: r.event.title,
      meta: r.event.category,
      path: route.fullPath,
    })
  },
  { immediate: true },
...
<template>
  <section class="event-detail-page">
    <div v-if="loading" class="loading-state">事件报告加载中...</div>
