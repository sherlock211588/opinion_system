...
class Pipeline:
    ...
    @staticmethod
    def _get_earliest_time(event_data: dict[str, Any]) -> str:
        """取事件 timeseries 第一条的时间"""
        ts = event_data.get("timeseries", [])
        if not ts:
            return ""
        return str(ts[0].get("time", ""))

    @staticmethod
    def _derive_source(event_data: dict[str, Any]) -> str:
        """从可用字段推导来源描述：
        优先 source/source_distribution，其次从 category 生成。
        """
        source = event_data.get("source", "").strip()
        if source:
            return source

        # 尝试从 source_distribution 提取 TOP 平台名
        sd = event_data.get("source_distribution")
        if isinstance(sd, list) and sd:
            top_names = [
                (item.get("name", "") if isinstance(item, dict) else str(item))
                for item in sd[:3]
            ]
            return " / ".join(filter(None, top_names))

        if isinstance(sd, dict) and sd:
            sorted_items = sorted(sd.items(), key=lambda x: float(x[1]) if isinstance(x[1], (int, float)) else 0, reverse=True)
            top_names = [name for name, _ in sorted_items[:3]]
    ...
    def event_report(
        ...
        report = {
            ...
            "source": self._derive_source(event_data),
            "event_time": self._get_latest_time(event_data),
            "start_time": self._get_earliest_time(event_data),
            "end_time": self._get_latest_time(event_data),
            "summary": event_data.get("summary", event_data.get("event_summary", event_data.get("event_title", ""))),
            "locations": event_data.get("locations", event_data.get("event_locations", "")),
            "keywords": event_data.get("keywords", event_data.get("event_keywords", "")),
        }
    ...
    def global_report(
        ...
        for eid, event_data in all_events.items():
            ...
            summary["summary"] = event_data.get("summary", event_data.get("event_summary", summary["event_title"]))
            summary["source"] = self._derive_source(event_data)
            summary["time"] = event_data.get("start_time") or summary.get("start_time", summary["event_time"])
            summary["start_time"] = self._get_earliest_time(event_data)
            summary["keywords"] = event_data.get("keywords", event_data.get("event_keywords", ""))
            summary["source_distribution"] = event_data.get("source_distribution", "")
